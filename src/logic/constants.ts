...
export const CONSTANTS = {
  ...
  CAREER_BAND_DATA: {
    ...
    // GRINDER: キャリア形状「粘り型」。能力ペナルティを軽減し、停滞耐性・長期滞在を重視。
    // abilityBias/-growthBias を下げることで B tier 以上が低確率でも十両に届く余地を確保。
    // retentionBias<1.0 で非関取でも在籍を続けやすく、stagnationBias<1.0 で停滞圧が緩やか。
    GRINDER: { weight: 26, abilityBias: -3, growthBias: -0.01, retentionBias: 0.90, stagnationBias: 0.96 },
  ...
  // 成長タイプごとの補正
  GROWTH_PARAMS: {
    // EARLY: peakEnd/decayStart を1年延ばし、B×EARLY が関取前に過剰に失速しないよう軽微調整。
    'EARLY': { peakStart: 20, peakEnd: 26, decayStart: 27, growthRate: 1.2 },
