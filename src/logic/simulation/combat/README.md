...
`boutFlowDiagnosticSnapshot.ts` composes a snapshot for reading `OpeningPhase -> ControlPhaseCandidate -> FinishRoute -> Kimarite` as one flow. It adds opening/control confidence, transition classification, diagnostic-only kimarite pattern evidence, and explanation-axis coverage (`FLOW_ONLY` / `FLOW_AND_RESULT` / `COMPLETE_CONTEXT`). Fixed-seed diagnostics still use it directly; production player bouts may also pass a completed snapshot into the commentary contract after battle resolution.

`boutFlowDiagnosticBuilder.ts` is the pure bridge from existing `BoutExplanationSnapshot` data to a completed diagnostic flow snapshot. It carries the already-selected kimarite pattern forward for commentary-only subfamily resolution, and derives victory-factor tags, hoshitori context tags, and banzuke context tags from already-computed values only. It does not inspect route weights, kimarite candidate weights, or consume RNG.
